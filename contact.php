...
include 'rate_limiting/rate_limiting.php';
...
if ($isFormSubmit) {
  ...
  // -------------------- SPAMPROTECTION ERROR MESSAGES END ----------------------
    ...
    $buttonClass = 'failed';
    $formMessage = '<img src="img/failed.png" style="width:25px;height:25px;vertical-align: middle;"> <span class="error_in_email_sending">Please check and correct your entries.</span>';
...
// redirect to success-page	   
		...
		if ($success) {
           ...
           $fehler['Sendmail'] = "<span class='errormsg-emailerror' style='display: block;line-height:25px;font-size:16px;'><span style='text-decoration:underline;font-size:18px;line-height:28px;color:red;'>The message could not be sent for one of the following reasons:</span><br /><ol style='text-align: left;line-height:25px;color:black;font-weight:normal;'>
  


 <li><b>You are using an external email address (e.g., from a free email provider).</b> We generally recommend using a domain-specific email address (e.g., info@your-domain.com), as an external email address (e.g., from GMX, Gmail, Yahoo, Web.de, Outlook, Freenet, Tuta, or Telekom/T-Online) is typically not permitted by a hosting provider. <br/><br/>  
   <span style=color:red;font-weight:bold;>Important:</span> Clear the cache and cookies (or website data) in your browser after you have entered the correct email address in the config.php file. Otherwise, this message may continue to appear. If needed, use an alternative browser to verify the functionality of the contact form.</li><br />
  
  <li><b>IONOS customers:</b> <a  style=\"color: blue; text-decoration: none;\" target=\"blank\" href=\"https://www.kontaktformular.com/en/web-host-hosteurope-strato.html#IONOS-E-Mail-Versand\">Please refer to Note #1.</a><br/><br/>
  
  <b>Hosteurope customers:</b> <a  style=\"color: blue; text-decoration: none;\" target=\"blank\" href=\"https://www.kontaktformular.com/en/web-host-hosteurope-strato.html#domain-eigene-email\">Please refer to Notes #2, #3, and #4.</a> (<span>&#8592; The information may also be relevant for other hosting providers!</span>)<br/><br/>
  
 
  
    <b>webgo customers:</b> <a  style=\"color: blue; text-decoration: none;\" target=\"blank\" href=\"https://www.kontaktformular.com/en/web-host-hosteurope-strato.html#domain-eigene-email\">Please refer to Note #2.</a> (<span>&#8592; The information may also be relevant for other hosting providers!</span>)<br/><br/>
    
    
   <b><a  style=\"color: blue; text-decoration: none;\" target=\"blank\" href=\"https://www.kontaktformular.com/en/hosting-provider-check.html\">Feel free to use our Hosting Provider Check page.</a></b> (&#8592; The contact form has been tested with various hosting providers.)<br/><br/>
  
  <span style=color:red;font-weight:bold;>Important:</span> Clear the cache and cookies (or website data) in your browser after making changes. Otherwise, this message may continue to appear. If necessary, use an alternative browser to verify the functionality of the contact form.</li><br />
  
  
  <li>The email address used (see config.php) may need to be verified with your hosting provider. Please contact your hosting provider.<br /><br /><b>Note:</b> Unfortunately, some hosting providers (e.g., Hosteurope, IONOS) prohibit the use of a non-domain email address (e.g., from free email providers). We therefore recommend using a domain-specific email address in the format info@your-domain.com.<br/><br/><span style=color:red;font-weight:bold;>Important:</span> Clear the cache and cookies (or website data) in your browser after entering the verified email address into the config.php file. Otherwise, this message may continue to appear. If needed, use an alternative browser to verify the functionality of the contact form.</li><br />
    
  
<li>You are running a local web server (http://localhost): Test the contact form on the web server of a reputable hosting provider. The PHP mail() function is usually only partially functional on a local web server.</li><br />
  
  
  <li>You have enabled SMTP in the config.php file and entered incorrect SMTP credentials. Please contact your hosting provider.<br /><br /><b>Note:</b> Enabling the SMTP function is not strictly necessary for sending emails, as the PHP mail() function is generally used.</li><br />
  
  
  <li>You have enabled SMTP in the config.php file and wish to connect to an external mail server (e.g., Gmail, GMX, WEB.DE, Yahoo, T-Online). You may need to request a port opening from your hosting provider. Please contact your hosting provider.<br /><br /><b>Note:</b> Enabling the SMTP function is not strictly necessary for sending emails, as the PHP mail() function is used by default.</li><br />
   
   
   <li><a  style=\"color: blue; text-decoration: none;\" target=\"blank\" href=\"https://www.kontaktformular.com/en/faq-script-php-contact-form.html#not-receive-email\">For more information, please visit our FAQ page. </a></li>
</ol>";
		}
    ...
    $formMessage = '<img src="img/finished.png" style="width:29px;height:29px;vertical-align: middle;"> <span class="successfully_sent">Your message has been sent.</span>';
...
<link href="css/material_icons/css.css" rel="stylesheet">
...
<p id="submitMessage" class="<?= $buttonClass ?>"><?= $formMessage ?><?php 
			...
			<div class="row">
				...
				<div class="col-sm-4 <?php if ($fehler["name"] != "") { echo 'error'; } ?> <?php echo (isset($_POST['name']) && ''!=$_POST['name'] ? 'not-empty-field ' : ''); ?>">
					...
					<input <?php if($cfg['HTML5_error_messages']) { ?> required  <?php }else{ ?> onchange="checkField(this)" <?php } ?> type="text" name="name" class="field" placeholder="Last Name *" value="<?php echo $_POST['name']; ?>" maxlength="<?php echo $number_of_characters_name; ?>" id="border-right5" onclick="setActive(this);" onfocus="setActive(this);"/>
		   ...
		   <div class="row">
				<div class="col-sm-8 <?php if ($fehler["subject"] != "") { echo 'error'; } ?> <?php echo (isset($_POST['subject']) && ''!=$_POST['subject'] ? 'not-empty-field ' : ''); ?>">
					...
					<input <?php if($cfg['HTML5_error_messages']) { ?> required  <?php }else{ ?> onchange="checkField(this)" <?php } ?> aria-label="Subject" type="text" name="subject" class="field" placeholder="Subject *" value="<?php echo $_POST['subject']; ?>" maxlength="<?php echo $number_of_characters_subject; ?>" id="border-right8" onclick="setActive(this);" onfocus="setActive(this);"/>
			...
			<div class="row">
				<div class="col-sm-8 <?php if ($fehler["message"] != "") { echo 'error'; } ?> <?php echo (isset($_POST['message']) && ''!=$_POST['message'] ? 'not-empty-field ' : ''); ?>">
					<label class="control-label textarea-label" for="border-right9"><i class="material-icons">message</i></label>
					<textarea <?php if($cfg['HTML5_error_messages']) { ?> required <?php }else{ ?> onchange="checkField(this)" <?php } ?> aria-label="Message" name="message" class="field" rows="5" placeholder="Message *" style="height:100%;width:100%;" id="border-right9" onclick="setActive(this);" onfocus="setActive(this);"><?php echo $_POST['message']; ?></textarea>
...
// -------------------- FILEUPLOAD START ----------------------
			if(0<$cfg['NUM_ATTACHMENT_FIELDS']){
				echo '<div class="row upload-row" style="background-position: 2.85rem center;-webkit-text-size-adjust:none;background-repeat: no-repeat;">
						<div class="col-sm-8">
							<label class="control-label" for="upload_field"><i id="fileupload-icon" class="fa fa-upload"></i></label>';
				for ($i=0; $i < $cfg['NUM_ATTACHMENT_FIELDS']; $i++) {
							echo '<input aria-label="Fileupload" type="file" size=12 name="f[]" id="upload_field" style="font-size:17px;" onclick="setActive(this);" onfocus="setActive(this);"/>';
				}
				echo '	</div>
					</div>';
			}
		...
		if($cfg['Security_code']) { ?>
			<div class="row captcha-row <?php if ($fehler["captcha"] != "") { echo 'error_container'; } ?>" style="background-position: 2.85rem center;-webkit-text-size-adjust:none;background-repeat: no-repeat;">
				<div class="col-sm-8 <?php if ($fehler["captcha"] != "") { echo 'error'; } ?>">
					...
					<div>
						<img aria-label="Captcha" src="captcha/captcha.php" alt="Security_code" title="kontaktformular.com-Security_code" id="captcha" />
					...
					<input <?php if($cfg['HTML5_error_messages']) { ?> required  <?php }else{ ?> onchange="checkField(this)" <?php } ?> aria-label="Eingabe" id="answer2" placeholder="Security code *" type="text" name="sicherheitscode" maxlength="150"  class="field<?php if ($fehler["captcha"] != "") { echo ' errordesignfields'; } ?>" onclick="setActive(this);" onfocus="setActive(this);" />
		...
		if($cfg['Security_question']) { ?>
			...
			<div class="row question-row <?php if ($fehler["q_id12"] != "") { echo 'error_container'; } ?>" style="background-position: 2.85rem center;-webkit-text-size-adjust:none;background-repeat: no-repeat;">
				<div class="col-sm-8 <?php if ($fehler["q_id12"] != "") { echo 'error'; } ?>">
					...
					<div aria-label="Security question">
						<?php echo $q[1]; ?>
						<input type="hidden" name="q_id" value="<?php echo $q[0]; ?>"/>
					</div>					
					<input <?php if($cfg['HTML5_error_messages']) { ?> required  <?php }else{ ?> onchange="checkField(this)" <?php } ?> aria-label="Answer" id="answer" placeholder="Answer *" type="text" class="field<?php if ($fehler["q_id12"] != "") { echo ' errordesignfields'; } ?>" name="q" onclick="setActive(this);" onfocus="setActive(this);" />
			...
			<div class="row checkbox-row" style="background-position: 2.85rem center;-webkit-text-size-adjust:none;background-repeat: no-repeat;">
				<div class="col-sm-8 <?php echo (isset($_POST['mail-copy']) && ''!=$_POST['mail-copy'] ? 'not-empty-field ' : ''); ?>">
					...
					<label class="checkbox-inline">
						<input aria-label="Send copy" type="checkbox" id="inlineCheckbox11" name="mail-copy" value="1" <?php if (isset($_POST['mail-copy']) && $_POST['mail-copy']=='1') echo(' checked="checked" '); ?> onclick="setActive(this);" onfocus="setActive(this);"> <div style="padding-top:4px;padding-bottom:2px;"><span style="line-height:27px;">Send a copy of the message by email</span></div>
					</label>
				</div>
			</div>
		<?php } 
		...
		if($cfg['Data_privacy_policy']) { ?>
			<div class="row checkbox-row <?php if ($fehler["data_protection"] != "") { echo 'error_container'; } ?>" style="background-position: 2.85rem center;-webkit-text-size-adjust:none;background-repeat: no-repeat;">
				<div class="col-sm-8 <?php if ($fehler["data_protection"] != "") { echo 'error'; } ?> <?php echo (isset($_POST['data_protection']) && ''!=$_POST['data_protection'] ? 'not-empty-field ' : ''); ?>">
					...
					<label class="checkbox-inline">
						<input <?php if($cfg['HTML5_error_messages']) { ?> required  <?php }else{ ?> onchange="checkField(this)" <?php } ?> aria-label="Data protection" type="checkbox" id="inlineCheckbox12" name="data_protection" value="accepted" <?php if ($_POST['data_protection']=='accepted') echo(' checked="checked" '); ?> onclick="setActive(this);" onfocus="setActive(this);"> <div style="padding-top:4px;padding-bottom:2px;line-height:27px;"> <a href="<?php echo "$dataprivacypolicy"; ?>" target="_blank">I accept the data privacy policy.</a> *</div>
					</label>
...
<span style="line-height:26px;font-size:17px;"><b>Note:</b> Fields marked with * are required.</span>
					<br />
					<br /><br />
...
<script type="text/javascript">document.addEventListener("DOMContentLoaded", () => {
    ...
    form.addEventListener('submit', function (event) {
        ...
        // Simulate the loading process and submit the form after one second.
        setTimeout(() => {
            form.submit(); // Perform the actual form submission after 1 second.
        }, 1000);
    });
});
...
<script>
    ...
    // Check if the "finished" class is active.
...
<div style="border: 1px solid #ccc; margin: 0;box-sizing: border-box;display: inline-block;line-height: 0.3;vertical-align: top;padding-bottom:7.5px;padding-right:5px;padding-left:5px;">
						<br /><br /><a href="https://www.kontaktformular.com/en" title="kontaktformular.com" style="text-decoration: none;color:#000000;font-size:13.5px;line-height: 20px;" target="_blank">&copy; by kontaktformular.com - All rights reserved.</a>
</div>
</div>
<!-- This copyright notice must not be removed. -->
